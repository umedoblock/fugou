#include "rsf.h"

/* parts functions */

static void _rsf_write_header_of_kernel(rs_file_t *rsf,
                                         uint bits,
                                         int poly,
                                         uint division)
{
    /* never change info */
    /* because it's kernel info */
    /* reed solomon info */
    fprintf(rsf->header, "0x%x\n", bits);
    fprintf(rsf->header, "0x%x\n", poly);
    /* rs info */
    fprintf(rsf->header, "0x%x\n", division);
    /* about text size */
    fprintf(rsf->header, "0x%x\n", rsf->text_size);

    /* separate, one empty line */
    fprintf(rsf->header, "\n");
}

static int _rsf_read_header_of_kernel(
                 FILE *header,
                 uint *bits,
                 uint *poly,
                 uint *division,
                 size_t *text_size)
{
    int ret = 0;
    char ss[RSF_SS_SIZE], *msg = NULL;

    /* fprintf(rsf->header, "0x%x\n", bits); */
    _fgets_(ss, RSF_SS_SIZE, header);
    sscanf(ss, "0x%x", bits);
    /* fprintf(rsf->header, "0x%x\n", poly); */
    _fgets_(ss, RSF_SS_SIZE, header);
    sscanf(ss, "0x%x", poly);
    /* fprintf(rsf->header, "0x%x\n", division); */
    _fgets_(ss, RSF_SS_SIZE, header);
    sscanf(ss, "0x%x", division);
    /* fprintf(rsf->header, "0x%x\n", ssb->text_size); */
    _fgets_(ss, RSF_SS_SIZE, header);
    sscanf(ss, "0x%x", text_size);

    /* fprintf(rsf->header, "\n"); */
    msg = fgets(ss, RSF_SS_SIZE, header);
    if (msg != NULL && strcmp(ss, "\n") == 0) {
        /* OK */
    }
    else {
        LOGGER(ERROR,
                   "failed to read new line code (='\\n'). "
                   "%s is invalid format.\n", ss);
        ret--;
    }

    return ret;
}

static int _rsf_read_header_of_hash(
                 char *header_hashed_name,
                 rs_file_t *rsf)
{
    char ss[RSF_SS_SIZE];
    int ret = 0;

    /* fprintf(rsf->header, "sha1\n"); */
    _fgets_(ss, RSF_SS_SIZE, rsf->header);
    if (strcmp("sha1", ss) == 0) {
        /* OK */
    }
    else {
        LOGGER(ERROR,
                   "_rsf_read_header_of_hash() failed.\n"
                   "failed to read \"sha1\" %s is "
                   "invalid format.\n\n", ss);
        ret--;
    }
    /* fprintf(rsf->header, "%s\n", ss); */
    _fgets_(ss, RSF_SS_SIZE, rsf->header);
    if (_ishashstring(ss, RSF_HASH_BIT_LENGTH)) {
        strcpy(header_hashed_name, ss);
    }
    else {
        LOGGER(ERROR,
                   "_rsf_read_header_of_hash() failed.\n"
                   "failed to read hashed_name. "
                   "%s is invalid format.\n\n", ss);
        ret--;
    }

    return ret;
}

rs_file_t *rsf_construct(uint bits, uint division, int build_type)
{
    reed_solomon_t *rs = NULL;
    char *mem = NULL, *mem_ = NULL;
    size_t grown_size = 0;
    rs_file_t rsf_, *rsf = NULL;
    rs_encode_t rse_, *rse = NULL;
    rs_decode_t rsd_, *rsd = NULL;
    int ret;

    LOGGER(DEBUG_, "in rsf_construct(bits=%u, division=%u, "
                        "build_type=%d).\n", bits, division, build_type);

    if (build_type != MODE_ENCODE && build_type != MODE_RECOVERY) {
        LOGGER(ERROR, "in rsf_construct() UNKNOWN build_type = %d\n",
                            build_type);
        return NULL;
    }

    ret = _rs_take_rs(&rs, bits, division);
    if (ret < 0)
        return NULL;

    rsf = &rsf_;
    rse = &rse_;
    rsd = NULL;
    if (build_type == MODE_RECOVERY) {
        rsd = &rsd_;
    }

    mem = _rsf_allocate(rsf, rsd, rse, rs, division);

    if (mem == NULL) {
        LOGGER(ERROR, "rsf_construct(bits=%u, division=%u, "
                           "build_type=%d) failed.\n",
                            bits, division, build_type);
        return NULL;
    }
    mem_ = mem;

    if (build_type == MODE_RECOVERY) {
        rsd = (rs_decode_t *)mem; mem += sizeof(rs_decode_t);
        _rs_calc_rsd_memory_size(rsd, rs, division);
        grown_size = _rs_init_rsd(rsd, mem, rs);
        mem += grown_size;
    }

    rse = (rs_encode_t *)mem; mem += sizeof(rs_encode_t);
    _rs_calc_rse_memory_size(rse, rs, division);
    grown_size = _rs_init_rse(rse, mem, rs);
    mem += grown_size;

    rsf = (rs_file_t *)mem; mem += sizeof(rs_file_t);
    rsf->mem = mem_;
    _rsf_calc_rsf_memory_size(rsf, division);
    grown_size = _rsf_init(rsf, mem, rsd, rse, build_type);
    mem += grown_size;

    #ifdef DEBUG
    _DEBUG("grown_size = %u\n", grown_size);
    _DEBUG("mem = %p\n", mem);

    _rs_view_rse(rse);
    _rsf_view_rsf(rsf);
    _DEBUG("mem - mem_ is %zu\n", mem - mem_);
    #endif

    if (rsd != NULL)
        _rs_view_rsd(rsd);
    if (rse != NULL)
        _rs_view_rse(rse);
    if (rsf != NULL)
        _rsf_view_rsf(rsf);

    _rs_make_vandermonde(rse);

    _DEBUG("\n\n");

    return rsf;
}

/* fopen() mode is "encode" or "decode" */
int rsf_decode_restored(char *restored_file_name, char *header)
{
    int ret = 0;
    rs_file_t *rsf = NULL;
    rs_decode_t *rsd = NULL;
    rs_encode_t *rse = NULL;
    reed_solomon_t *rs = NULL;
    _target_size_brother_t *ssb = NULL;
    uint symbols_in_target, symbols_in_buf;
    uint remained_symbols, available_symbols;
    size_t r, w;

    restored_file_name[0] = '\0';

    ret = _rsf_get_up_rsf_for_restored(&rsf, header);
    if (ret < 0)
        return ret;
    _DEBUG("in rsf_decode_restored()\n");
    _DEBUG("rs_get_up_rsf_for_recover() = %d ok.\n\n", ret);

    rse = rsf->rse;
    rsd = rsf->rsd;
    rs = rsd->rs;
    ssb = rsf->ssb;

    /* rank error, row swap を試してみること。 */
    _rsf_merge_slots(rsd, rsf, rse);
    _rs_solve_inverse(rsd->inverse, rsd->merged, rsd);

    symbols_in_target = rsf->symbols_in_target;
    symbols_in_buf = rsf->symbols_in_buf;

    LOGGER(ERROR, "in rsf_decode_restored().\n");

    if (symbols_in_target < symbols_in_buf) {
        available_symbols = symbols_in_target;
    }
    else {
        available_symbols = symbols_in_buf;
    }
    remained_symbols = symbols_in_target;
    while (remained_symbols) {
        r = _fread_from_slots(rsf->merged,
                              rs->symbol_size,
                              available_symbols,
                              rsd->division);
        if (r != available_symbols) {
            LOGGER(ERROR, "rsf_decode_restored() failed.\n");
            LOGGER(ERROR, "cause of that _fread_from_slots()\n");
            ret = RSF_FREAD_ERROR;
            goto err_fread;
        }
        _rs_decode_slots(rsf, rsd, available_symbols);
        w = _fwrite_to_slots(rsf->recover,
                             rs->symbol_size, available_symbols,
                             rsd->division);
        if (w != available_symbols) {
            LOGGER(ERROR, "rsf_decode_restored() failed.\n");
            LOGGER(ERROR, "cause of that _fwrite_to_slots()\n");
            ret = RSF_FWRITE_ERROR;
            goto err_fwrite;
        }
        remained_symbols = _update_remained_symbols(
                                remained_symbols, &available_symbols);
    }

    ret = _rsf_recover_restored(rsf, symbols_in_target, symbols_in_buf);
    if (ret < 0) {
        LOGGER(ERROR, "_rsf_recover_restored(symbols_in_target=%u, "
                           "symbols_in_buf=%u) failed.\n",
                            symbols_in_target, symbols_in_buf);
        goto err_recover;
    }

    fflush(rsf->restored);
    fseek(rsf->restored, 0L, SEEK_SET);
    sha1_as_file(rsf->sha1sum, rsf->restored);
    _to_hashed_name(restored_file_name, rsf->sha1sum);
    ret = RSF_SCUCCESS;

    _DEBUG("in rsf_decode_restored()\n");
    _DEBUG("rs_recover_file() done.\n\n");

err_recover:
err_fwrite:
err_fread:
    _rsf_good_night_rsf_for_recover(rsf, restored_file_name);

    return ret;
}

int _rsf_get_up_rsf_for_restored(rs_file_t **_rsf, char *header_path)
{
    char text_path[RSF_SS_SIZE];
    char header_hashed_name[RSF_SS_SIZE];
    int ret = 0;
    uint bits = 0, poly = 0, division = 0;
    uint len_available_slots = 0;
    FILE *header = NULL;
    size_t text_size = 0;
    rs_file_t *rsf = NULL;
    rs_decode_t *rsd = NULL;
    reed_solomon_t *rs = NULL;

    _DEBUG("in _rsf_get_up_rsf_for_restored()\n");

    header = fopen(header_path, "r");
    _DEBUG("fopen(header_path=\"%s\", \"r\")=%p\n", header_path, header);
    if (header == NULL) {
        LOGGER(ERROR, "rs_get_up_rsf_for_recover() failed.\n");
        /* tekito */
        ret = -203;
        goto err_no_resource;
    }

    ret = _rsf_read_header_of_kernel(header,
                           &bits, &poly, &division, &text_size);
    if (ret < 0) {
        LOGGER(ERROR, "rs_get_up_rsf_for_recover() failed.\n");
        LOGGER(ERROR, "ret = %d, header = %p\n", ret, header);
        LOGGER(ERROR, "bits=%u, poly=%u, division=%u, "
                           "text_size=%zu\n\n",
                            bits, poly, division, text_size);
        ret = -204;
        goto err_after_open_header;
    }
    _DEBUG("_rsf_read_header_of_kernel() = %d ok.\n", ret);

    ret = _rsf_make_decoder(_rsf, bits, division);
    if (ret < 0) {
        LOGGER(ERROR, "rs_get_up_rsf_for_recover() failed.\n");
        LOGGER(ERROR, "ret = %d, header = %p\n", ret, header);
        LOGGER(ERROR, "cannot make recoveryer. "
                           "bits=%u, poly=%u, division=%u, "
                           "text_size=%zu\n",
                            bits, poly, division, text_size);
        ret = -205;
        goto err_after_open_header;
    }
    _DEBUG("_rsf_make_decoder() = %d ok.\n", ret);

    /* must be here to set *rsf->header after _rsf_make_decoder() */
    rsf = *_rsf;
    rsf->header = header;

    rsd = rsf->rsd;
    rs = rsd->rs;

    ret = _rsf_read_header_of_hash(header_hashed_name, rsf);
    if (ret < 0) {
        LOGGER(ERROR, "_rsf_read_header_of_hash() failed.\n");
        LOGGER(ERROR,
                   "bits=%u, poly=%u, division=%u, "
                   "text_size=%zu, hash=\"%s\"\n",
                    bits, poly, division, text_size, header_hashed_name);
        goto err_after_open_header;
    }
    else {
        _DEBUG("_rsf_read_header_of_hash() = %d\n", ret);
    }
    _DEBUG("text hashed name is\n");
    _DEBUG("\"%s\"\n", header_hashed_name);

    strcpy(text_path, header_hashed_name);
    ret = _rsf_files_open(&len_available_slots, rsf, text_path, "wb+");
    if (ret < 0) {
        goto err_after_open_header;
    }
    if (len_available_slots < rsd->division) {
        LOGGER(ERROR, "_rsf_files_open() failed.\n");
        LOGGER(ERROR, "len_available_slots < rsd->division\n");
        LOGGER(ERROR, "len_available_slots=%u\n", len_available_slots);
        LOGGER(ERROR, "rsd->division=%u\n", rsd->division);
        ret = RSF_LACK_OF_SLOTS_ERROR;
        goto err_after_rsf_file_open;
    }

    /* BUG: 多分ここで死ぬ。だって、mergedって実態がないもの */
    ret = slot_calc_and_set_target_size(rsf->merged,
                                    rs->symbol_size,
                                    text_size,
                                    division);
    ret = _rsf_calc_symbols(rsf, rs->symbol_size, rsf->target_size, buf_size);
    if (ret < 0)
        goto err_after_rsf_file_open;

    return RSF_SCUCCESS;

    err_after_rsf_file_open:
        _rsf_files_close(rsf, rsd->division);
    err_after_open_header:
        fclose(rsf->header);
        rsf->header = NULL;
        _DEBUG("fclose(header=%p) header_path=\"%s\"\n",
                          header,  header_path);
    err_no_resource:
    LOGGER(ERROR, "header_path=\"%s\"\n", header_path);
    _DEBUG("\n\n");

    return ret;
}

static int _rsf_good_night_rsf_for_recover(rs_file_t *rsf,
                                           char *recovery_file_name)
{
    rs_decode_t *rsd = rsf->rsd;

    fclose(rsf->header);
    _rsf_files_close(rsf, rsd->division);
    _rsf_free(rsf);
    _DEBUG("in _rsf_good_night_rsf_for_recover()\n");
    _DEBUG("closed recovery_file=\"%s\"\n\n", recovery_file_name);

    return RSF_SCUCCESS;
}

static uint _rsf_get_division(rs_file_t *rsf)
{
    uint division = 0;

    if (rsf->rsd != NULL)
        division = rsf->rsd->division;
    else if (rsf->rse != NULL)
        division = rsf->rse->division;
    else {
        LOGGER(ERROR,  "_rsf_get_division() failed.\n"
                            "cause of that rsd and rse are NULL pointer.\n\n");
        return RSF_DIVISION_ERROR;
    }

    return division;
}

static void _rsf_view_rsf(rs_file_t *rsf)
{
    uint i, division = 0;
    char ss[RSF_SS_SIZE];
    sha1sum_t sha1sum[1];

    division = _rsf_get_division(rsf);

    LOGGER(INFO, "rsf = %p\n", rsf);
    LOGGER(INFO, "                 mem = %p\n", rsf->mem);
    LOGGER(INFO, "                 text = %p\n", rsf->text);
    LOGGER(INFO, "             restored = %p\n", rsf->restored);
    _to_hashed_name(ss, sha1sum);
    LOGGER(INFO, "      sha1sum = %p = %s\n", rsf->sha1sum, ss);
    LOGGER(INFO, "\n");

    LOGGER(INFO, "        text_size = %u\n", rsf->text_size);
    LOGGER(INFO, "      symbols_in_target = %zu\n", rsf->symbols_in_target);
    LOGGER(INFO, "    symbols_in_buf = %zu\n", rsf->symbols_in_buf);
    LOGGER(INFO, "\n");

    LOGGER(INFO, "              mode = %d is ", rsf->mode);
    if (rsf->mode == MODE_ENCODE)
        LOGGER(INFO, "ENCODE");
    else if (rsf->mode == MODE_RECOVERY)
        LOGGER(INFO, "RECOVERY");
    else
        LOGGER(INFO, "UNKNOWN");
    LOGGER(INFO, "         temp_path = %p, \"%s\"\n",
                          rsf->temp_path, rsf->temp_path);
    LOGGER(INFO, "         temp_tail = %p, \"%s\"\n",
                          rsf->temp_tail, rsf->temp_tail);
    LOGGER(INFO, "          dir_name = \"%s\"\n", rsf->dir_name);
    LOGGER(INFO, "         base_name = \"%s\"\n", rsf->base_name);
    LOGGER(INFO, "            header = %p\n", rsf->header);
    LOGGER(INFO, "\n");

    rs_view_rse(rsf->rse);
    rs_view_rsd(rsf->rsd);
    for (i=0;i<division;i++)
        LOGGER(INFO, "    norm[%5d].target = %p\n",
                                     i, rsf->norm[i].target);
    for (i=0;i<division;i++)
        LOGGER(INFO, "  parity[%5d].target = %p\n",
                                     i, rsf->parity[i].target);
    for (i=0;i<division;i++)
        LOGGER(INFO, "  merged[%5d].target = %p\n",
                                     i, rsf->merged[i].target);
    for (i=0;i<division;i++)
        LOGGER(INFO, " recover[%5d].target = %p\n",
                                     i, rsf->recover[i].target);

    LOGGER(INFO, "     allocate_size = %u\n", rsf->allocate_size);
    LOGGER(INFO, "        slots_size = %u\n", rsf->slots_size);
    LOGGER(INFO, "temp_path_max_size = %u\n", rsf->temp_path_max_size);
    LOGGER(INFO, " dir_name_max_size = %u\n", rsf->dir_name_max_size);
    LOGGER(INFO, "    base_name_size = %u\n", rsf->base_name_size);
    LOGGER(INFO, "\n");
}

/* must set rsf->buf_size before below function call. */
static size_t _rsf_calc_rsf_memory_size(rs_file_t *rsf, uint division)
{
    size_t hex_name_size = 0;

    rsf->hash_size = RSF_HASH_SIZE;
    LOGGER(FATAL, "rsf->buf_size = %u\n", rsf->buf_size);
    rsf->buf_size = RSF_BUF_SIZE;
    /* buffer and slots need three type. there are norm, parity, merged */
    rsf->slots_size = division * sizeof(slot_t);
    rsf->temp_path_max_size = RSF_PATH_MAX_SIZE;

    rsf->hash_size = aligned_size(rsf->hash_size);
    rsf->buf_size = aligned_size(rsf->buf_size);
    rsf->slots_size = aligned_size(rsf->slots_size);
    rsf->temp_path_max_size = aligned_size(rsf->temp_path_max_size);

    /* SHA1("abc") is equal to "a9993e364706816aba3e25717850c26c9cd0d89d"  */
    /* hash_name = SHA1("abc") */
    hex_name_size = rsf->hash_size * 2;
    /* base_name = tmpnam() + "." + "0x04x" + '\0'  = L_tmpnam(=20) + 1 + 6 + 1
     *             or
     *             "h" + SHA1("abc") + '\0' = 1 + 40 + 1
     */
    rsf->base_name_size = 1 + hex_name_size + 1;
    rsf->base_name_size = aligned_size(rsf->base_name_size);
    /* temp_path = dir_name + "/" + base_name */
    /* therefore need to decrement dir_name_max_size for "/" */
    rsf->dir_name_max_size = rsf->temp_path_max_size - rsf->base_name_size - 1;
    rsf->dir_name_max_size = aligned_size(rsf->dir_name_max_size);

    rsf->allocate_size = sizeof(rs_file_t) +
                         rsf->slots_size * 4 +
                         rsf->buf_size * division * 4 +
                         rsf->temp_path_max_size +
                         rsf->base_name_size +
                         rsf->dir_name_max_size +
                         0; /* no mean */
    return rsf->allocate_size;
}

static size_t _rsf_init(rs_file_t *rsf,
                        char *mem,
                        rs_decode_t *rsd,
                        rs_encode_t *rse,
                        int mode)
{
    uint i, division = 0;
    char *mem_ = mem;

    rsf->rsd = rsd;
    rsf->rse = rse;

    division = _rsf_get_division(rsf);

    rsf->mode = mode;

    rsf->norm = (slot_t *)mem;     mem += rsf->slots_size;
    rsf->parity = (slot_t *)mem;   mem += rsf->slots_size;
    rsf->merged = (slot_t *)mem;   mem += rsf->slots_size;
    rsf->recover = (slot_t *)mem; mem += rsf->slots_size;

    rsf->temp_path = mem;         mem += rsf->temp_path_max_size;
    rsf->dir_name = mem;          mem += rsf->dir_name_max_size;
    rsf->base_name = mem;         mem += rsf->base_name_size;

    for (i=0;i<division;i++) {
        rsf->norm[i].buf = (uchar *)mem;
        mem += rsf->buf_size;
    }
    for (i=0;i<division;i++) {
        rsf->parity[i].buf = (uchar *)mem;
        mem += rsf->buf_size;
    }
    for (i=0;i<division;i++) {
        rsf->merged[i].buf = (uchar *)mem;
        mem += rsf->buf_size;
    }
    for (i=0;i<division;i++) {
        rsf->recover[i].buf = (uchar *)mem;
        mem += rsf->buf_size;
    }

    return (size_t )(mem - mem_);
}

static size_t _rsf_calc_memory_size(
                    rs_file_t *rsf,
                    rs_decode_t *rsd,
                    rs_encode_t *rse,
                    reed_solomon_t *rs,
                    uint division)
{
    size_t allocate_size = 0;

    LOGGER(DEBUG_, "in _rsf_calc_memory_size().\n");
    if (rsf != NULL) {
        _rsf_calc_rsf_memory_size(rsf, division);
        allocate_size += rsf->allocate_size;
        LOGGER(DEBUG_, "rsf->allocate_size = %u\n", rsf->allocate_size);
    }
    if (rsd != NULL) {
        _rs_calc_rsd_memory_size(rsd, rs, division);
        allocate_size += rsd->allocate_size;
        LOGGER(DEBUG_, "rsd->allocate_size = %u\n", rsd->allocate_size);
    }
    if (rse != NULL) {
        _rs_calc_rse_memory_size(rse, rs, division);
        allocate_size += rse->allocate_size;
        LOGGER(DEBUG_, "rse->allocate_size = %u\n", rse->allocate_size);
    }
    LOGGER(DEBUG_, "allocate_size = %u\n", allocate_size);

    return allocate_size;
}

static char *_rsf_allocate(
                   rs_file_t *rsf,
                   rs_decode_t *rsd,
                   rs_encode_t *rse,
                   reed_solomon_t *rs,
                   uint division)
{
    size_t allocate_size = 0;
    char *mem = NULL;

    allocate_size = _rsf_calc_memory_size(rsf, rsd, rse, rs, division);

    mem = (char *)malloc(allocate_size);
    if (mem == NULL) {
        LOGGER(ERROR, "_rsf_allocate() failed.\n");
        LOGGER(ERROR, "mem = %p, malloc(allocate_size=%u)\n",
                           mem, allocate_size);
    }
    else {
        memset(mem, '\0', allocate_size);
        LOGGER(INFO, "_rsf_allocate() success.\n");
        LOGGER(INFO, "mem = %p, malloc(allocate_size=%u)\n",
                          mem, allocate_size);
    }

    return mem;
}

static int _rsf_make_encoder(rs_file_t **_rsf, uint bits, uint division)
{
    rs_file_t *rsf;

    rsf = rsf_construct(bits, division, MODE_ENCODE);
    if (rsf == NULL) {
        LOGGER(ERROR, "rsf_construct(bits=%u, division=%u, build_type=MODE_ENCODE) failed.\n", bits, division);
        return RSF_CONSTRUCT_ERROR;
    }
    *_rsf = rsf;
    return RSF_SCUCCESS;
}

static int _rsf_make_decoder(rs_file_t **_rsf, uint bits, uint division)
{
    rs_file_t *rsf = NULL;

    _DEBUG("in _rsf_make_decoder()\n");

    rsf = rsf_construct(bits, division, MODE_RECOVERY);
    if (rsf == NULL) {
        LOGGER(ERROR, "rsf_construct(bits=%u, division=%u, build_type=MODE_RECOVERY) failed.\n", bits, division);
        return RSF_CONSTRUCT_ERROR;
    }

    *_rsf = rsf;
    return RSF_SCUCCESS;
}

static int _rsf_free(rs_file_t *rsf)
{
    LOGGER(DEBUG_, "in _rsf_free(rsf=%p).\n", rsf);
    LOGGER(DEBUG_, "free(rsf->mem=%p).\n\n", rsf->mem);
    free(rsf->mem);

    return RSF_SCUCCESS;
}

static void _to_hashed_name(char *ss, sha1sum_t *sha1sum)
{
    sprintf(ss, "h");
    sha1_get_hex(ss + 1, sha1sum);
}

static int _refine_base_name(char *temp_path)
{
    int i;
    char *last_sep = NULL;
    for (i=0;temp_path[i] != '\0';i++) {
        if (temp_path[i] == '/' ||
            temp_path[i] == '\\' )
            last_sep = temp_path + i;
    }
    if (last_sep)
        strcpy(temp_path, last_sep + 1);
    return RSF_SCUCCESS;
}

static char *_make_tmpnam(rs_file_t *rsf)
{
    char *p = NULL;

    p = tmpnam(rsf->temp_path);
    /*
    mkstemp();
    tmpfile();
    */
    _refine_base_name(rsf->temp_path);
    rsf->temp_tail = rsf->temp_path + strlen(rsf->temp_path);
    return p;
}

static FILE *_fopen_rse(rs_file_t *rsf,
                        const char *mode,
                        int no)
{
    FILE *fp;
    if (rsf->temp_path[0] == '\0')
        _make_tmpnam(rsf);

    sprintf(rsf->temp_tail, ".0x%08x", no);
    fp = fopen(rsf->temp_path, mode);
    return fp;
}

/* header file にて、改行codeとするのは、'\n' == 0x0a のみ。
header fileのsha1sumの値をfile名に含める関係上、
改行codeについて明確にしなければならなかった。

'\r' == 0x0d は改行codeとしてheader file内に存在してはいけない。
header file 内に '\r' == 0x0d が含まれていたのならば、
そのheaderは不正なheader fileである。
*/
static char *_fgets_(char *s, int size, FILE *stream)
{
    int i;
    char *p = NULL;
    p = fgets(s, size, stream);
    for (i=0;p && i<size;i++) {
        if (s[i] == '\n') {
            s[i] = '\0';
            break;
        }
    }
    return p;
}

static uint _rsf_norm_files_open(
                 rs_file_t *rsf,
                 uint division)
{
    uint i, available = 0;
    char hashed_name[RSF_SS_SIZE];
    for (i=0;i<division;i++) {
        if (rsf->mode == MODE_ENCODE)
            rsf->norm[i].target = _fopen_rse(rsf, "wb+", i);
        else {
            /* rsf->mode == MODE_RECOVERY */
            _fgets_(hashed_name, RSF_SS_SIZE, rsf->header);
            rsf->norm[i].target = fopen(hashed_name, "rb");
            if (rsf->norm[i].target == NULL) {
                rsf->recover[i].target = fopen(hashed_name, "wb+");
            }
            else {
                rsf->recover[i].target = NULL;
            }
        }
        if (rsf->norm[i].target != NULL)
            available++;
    }
    return available;
}

static uint _rsf_parity_files_open(rs_file_t *rsf,
                                   uint division)
{
    uint i, available = 0;
    char hashed_name[RSF_SS_SIZE];
    for (i=0;i<division;i++) {
        if (rsf->mode == MODE_ENCODE)
            rsf->parity[i].target = _fopen_rse(rsf, "wb+", division + i);
        else {
            /* rsf->mode == MODE_RECOVERY */
            _fgets_(hashed_name, RSF_SS_SIZE, rsf->header);
            rsf->parity[i].target = fopen(hashed_name, "rb");
        }
        if (rsf->parity[i].target != NULL)
            available++;
    }
    return available;
}

static int _rsf_files_open(uint *len_available_slots,
                           rs_file_t *rsf,
                           const char *text_path,
                           const char *mode)
{
    uint division = 0;
    uint len_norm_slots, len_parity_slots;

    _DEBUG("in _rsf_files_open()\n");
    if (rsf->mode == MODE_ENCODE) {
        _DEBUG("rsf->mode == MODE_ENCODE.\n");
    }
    else {
        _DEBUG("rsf->mode == MODE_RECOVERY.\n");
    }

    division = _rsf_get_division(rsf);
    if (rsf->mode == MODE_ENCODE ) {
        rsf->text = fopen(text_path, mode);
        if (rsf->text == NULL) {
            LOGGER(ERROR,
                       "_rsf_files_open() failed.\n"
                       "cannot open \"%s\" "
                       "to make rs file.\n", text_path);
            return RSF_INVALID_PATH_ERROR;
        }
    }
    else if (rsf->mode == MODE_RECOVERY) {
        rsf->restored = fopen(text_path, mode);
        _DEBUG("rsf->restored = fopen(text_path=\"%s\", mode=\"%s\")\n",
                                            text_path, mode);
        _DEBUG("rsf->restored = %p\n", rsf->restored);
        if (rsf->restored == NULL) {
            LOGGER(ERROR, "_rsf_files_open() failed.\n");
            LOGGER(ERROR, "cannot open \"%s\"\n", text_path);
            LOGGER(ERROR, "to recover rs file.\n");
            return RSF_INVALID_PATH_ERROR;
        }
    }
    else {
        LOGGER(ERROR, "_rsf_files_open() failed.\n");
        LOGGER(ERROR, "rsf->mode value(=%d) is unknown.\n", rsf->mode);
        LOGGER(ERROR, "\n");
        return RSF_INVALID_MODE_ERROR;
    }

    if (rsf->mode == MODE_ENCODE) {
        sha1_as_file(rsf->sha1sum, rsf->text);
        rsf->text_size = rsf->sha1sum->message_size;
        /* need to move file top position for making norm slot. */
        rewind(rsf->text);
    }

    len_norm_slots = _rsf_norm_files_open(rsf, division);
    len_parity_slots = _rsf_parity_files_open(rsf, division);
    *len_available_slots = len_norm_slots + len_parity_slots;

    _DEBUG("\n\n");

    return RSF_SCUCCESS;
}

static void _rsf_files_close(rs_file_t *rsf, uint division)
{
    uint i;

    _DEBUG("in _rsf_files_close()\n");
    for (i=0;i<division;i++) {
        if (rsf->norm[i].target != NULL)
            fclose(rsf->norm[i].target);
    }
    for (i=0;i<division;i++) {
        if (rsf->parity[i].target != NULL)
            fclose(rsf->parity[i].target);
    }
    /* no need to call fclose() for rsf->merged[i].target.
     * rsf->mode == MODE_ENCODE then
     * no use rsf->merged[i].
     *
     * rsf->mode == MODE_RECOVERY then
     * since
     * rsf->merged[i] = rsf->norm[i];
     * or
     * rsf->merged[i] = rsf->parity[j];
     * in _rsf_merge_slots().
     */
    for (i=0;i<division;i++) {
        if (rsf->recover[i].target != NULL)
            fclose(rsf->recover[i].target);
    }

    if (rsf->text != NULL)
        fclose(rsf->text);
    if (rsf->restored != NULL)
        fclose(rsf->restored);
    if (p2f->header_file != NULL)
        fclose(p2f->header_file);

    _DEBUG("\n\n");
}

uint _update_remained_symbols(uint remained_symbols,
                                     uint *available_symbols)
{
    LOGGER(ERROR, "in _update_remained_symbols()\n");
    _DEBUG("in _update_remained_symbols()\n");
    _DEBUG("remained_symbols = %u, *available_symbols = %u\n\n",
                 remained_symbols, *available_symbols);

    LOGGER(ERROR,
               "remained_symbols = %u, *available_symbols = %u\n",
                remained_symbols, *available_symbols);

    remained_symbols -= *available_symbols;
    if (remained_symbols < *available_symbols) {
        *available_symbols = remained_symbols;
    }

    LOGGER(ERROR,
               "remained_symbols = %u, *available_symbols = %u\n\n",
                remained_symbols, *available_symbols);

    return remained_symbols;
}

static int _rsf_write_header_of_hash(rs_file_t *rsf,
                                     uint division)
{
    char hashed_name[RSF_SS_SIZE];
    uint i;

    /* hash part */
    /* maybe change below value if cracked hash function. */

    /* about text hashed function */
    fprintf(rsf->header, "sha1\n");

    /* about text hash value */
    _to_hashed_name(hashed_name, rsf->sha1sum);
    fprintf(rsf->header, "%s\n", hashed_name);

    /* about norm slot sha1sum */
    for (i=0;i<division;i++) {
        _rename_to_hashed_name(hashed_name, rsf, i);
        fprintf(rsf->header, "%s\n", hashed_name);
    }
    /* about parity slot sha1sum */
    for (i=0;i<division;i++) {
        _rename_to_hashed_name(hashed_name, rsf, division + i);
        fprintf(rsf->header, "%s\n", hashed_name);
    }

    return RSF_SCUCCESS;
}

/* fopen() mode is "encode" or "decode" */
int rsf_encode_text(char *hashed_header,
                    const char *path,
                    uint bits,
                    uint division)
{
    int ret;
    rs_file_t *rsf = NULL;
    rs_encode_t *rse = NULL;
    reed_solomon_t *rs = NULL;
    _target_size_brother_t *ssb = NULL;
    uint len_available_slots = 0, available_symbols, remained_symbols;
    uint symbols_in_target = 0, symbols_in_buf = 0;
    size_t r = 0, w = 0;

    hashed_header[0] = '\0';

    ret = _rsf_make_encoder(&rsf, bits, division);
    if (ret < 0)
        return ret;

    rse = rsf->rse;
    rs = rse->rs;

    ret = _rsf_files_open(&len_available_slots, rsf, path, "rb");
    if (ret < 0) {
        _rsf_free(rsf);
        return ret;
    }

    ret = slot_calc_and_set_target_size(rsf->norm,
                                    rs->symbol_size,
                                    rsf->text_size,
                                    division);
    ret = _rsf_calc_symbols(rsf,
                            rs->symbol_size,
                            ssb->target_size,
                            rsf->buf_size);

    rsf->header = _fopen_rse(rsf, "wb", 2 * division);
    _rsf_write_header_of_kernel(rsf,
                                rs->bits, rs->poly, division);

    /* PETA(too large) size file and slots make too large size slot.
     * symbols_in_target = TERA = PETA / number of slots
     * symbols_in_beffer = also from 1024 to 4096 octets.
     * then, symbols_in_target >>>>>>> symbols_in_buf
     */
    symbols_in_target = rsf->symbols_in_target;
    symbols_in_buf = rsf->symbols_in_buf;

    ret = _rsf_split_text(rsf, rs->symbol_size, division,
                          symbols_in_target, symbols_in_buf);
    if (ret < 0) {
        _rsf_free(rsf);
        return ret;
    }

    LOGGER(WARN, "in _rs_encode_slots_file()\n");
    if (symbols_in_target < symbols_in_buf) {
        available_symbols = symbols_in_target;
    }
    else {
        available_symbols = symbols_in_buf;
    }
    remained_symbols = symbols_in_target;
    /* slot.c に移植するつもり。*/
    while (remained_symbols) {
        r = _fread_from_slots(rsf->norm,
                              rs->symbol_size,
                              available_symbols,
                              division);
        rs_encode_slots(rsf->parity,
                        rsf->norm,
                        rse,
                        available_symbols);
        w = _fwrite_to_slots(rsf->parity,
                             rs->symbol_size,
                             available_symbols,
                             division);
        remained_symbols = _update_remained_symbols(remained_symbols,
                                                   &available_symbols);
    }

    _rsf_files_close(rsf, division);
    _rsf_write_header_of_hash(rsf, division);
    fclose(rsf->header);

    _rename_to_hashed_name(hashed_header, rsf, 2 * division);

    _rsf_free(rsf);

    return RSF_SCUCCESS;
}

int _rsf_calc_symbols(rs_file_t *rsf,
                      size_t symbol_size,
                      size_t target_size,
                      size_t buf_size)
{
    rsf->symbols_in_target = target_size / symbol_size;
    rsf->symbols_in_buf = buf_size / symbol_size;

    _DEBUG("in _rsf_calc_symbols()\n");
    _DEBUG("buf_size = %u\n", buf_size);
    _DEBUG("target_size = %u\n", target_size);
    _DEBUG("symbol_size = %u\n", symbol_size);
    _DEBUG("symbols_in_target = %u\n", rsf->symbols_in_target);
    _DEBUG("symbols_in_buf = %u\n", rsf->symbols_in_buf);
    _DEBUG("\n");

    return RSF_SCUCCESS;
}

static int _rename_to_hashed_name(char *hashed_name,
                                  rs_file_t *rsf,
                                  int no)
{
    sha1sum_t sha1sum[1];
    FILE *fp;
    int ret;

    fp = _fopen_rse(rsf, "rb", no);
    if (fp == NULL) {
        LOGGER(ERROR,
                   "_fopen_rse(%p, \"rb\", %d) = %p "
                   "in _rename_to_hashed_name()\n",
                    rsf, no, fp);
        return -1;
    }
    sha1_as_file(sha1sum, fp);
    fclose(fp);
    _to_hashed_name(hashed_name, sha1sum);
    ret = rename(rsf->temp_path, hashed_name);
    if (ret < 0) {
        LOGGER(ERROR, "rename(\"%s\", \"%s\") failed.\n\n",
                            rsf->temp_path, hashed_name);
        perror(NULL);
    }
    else {
        _DEBUG("in _rename_to_hashed_name()\n");
        _DEBUG("rename(\"%s\", \"%s\")\n\n", rsf->temp_path, hashed_name);
    }
    return RSF_SCUCCESS;
}

/* ground functions */

/* int(math.log(gf_max, 10)) + 1 */
int _calc_log10(int n)
{
    int digits = 0;

    while (n > 0){
        n /= 10;
        digits++;
    }

    return digits;
}

static int _ishashstring(char *hashed_string, uint hash_len)
{
    int ch, ret = -1;
    size_t len = 0, i;

    /* hashed_string = "h" + SHA1() */
    len = strlen(hashed_string);
    if (len == 1 + hash_len / 8 * 2 && hashed_string[0] == 'h') {
        /* OK */

        for(i=1;i<len;i++) {
            ch = hashed_string[i];
            if ((ch >= '0' && ch <= '9') ||
                (ch >= 'a' && ch <= 'f'))
                /* OK */
                ;
            else {
                ret = 0;
                break;
            }
        }
        if (i == len)
            ret = 1;
    }
    else {
        ret = 0;
    }

    return ret;
}

